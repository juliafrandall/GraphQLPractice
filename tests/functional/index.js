...
define([
    ...
    '../support/pages/ConsumerToolsPage',
    '../support/pages/NutritionInfoPage',
    '../support/pages/ContactPage'
], function(fs, intern, registerSuite, assert, IndexPage, ConsumerToolsPage, NutritionInfoPage, ContactPage) {
    registerSuite(function() {
        var indexPage;
        var consumerToolsPage;
        ...
        return {
            setup: function() {
                ...
                consumerToolsPage = new ConsumerToolsPage(this.remote);
            ...
            'navigable bottom menu': function() {
              if (config.mobileDevice == true) {
                ...
                return indexPage
                    ...
                    .then(function(bool) {
                        assert.isTrue(bool,
                            'Bottom menu should be navigable');
                    });
              }
            },
            // 'usable search box': function() {
            //     if (config.mobileDevice == true) {
            //         this.skip('desktop-only test');
            //     } else {
            //         var foodName = 'wine';
            ...
            '404 item page': function() {
                if (config.skip !=true ){
                  if (config.mobileDevice == true) {
                      this.skip('desktop-only test');
                  } else {
                      return nutritionInfoPage
                          .GroceryItem404(config)
                          .then(function(bool) {
                              assert.isTrue(bool,
                                  '404 item page top have correct error page');
                          });
                  }
                } else {
                  this.skip('disabled by developer');
                }
             ...
             * Consumer Tools
             */
             'track app page': function() {
                 if (config.mobileDevice == true) {
                     this.skip('desktop-only test');
                 } else {
                     return consumerToolsPage
                         .trackApp(config)
                         .then(function(bool) {
                             assert.isTrue(bool,
                                 'Track App page should be navigable');
                         });
                 }
             },
            'navigable popular foods': function() {
                if (config.mobileDevice == true) {
                    this.skip('desktop-only test');
                } else {
                    return consumerToolsPage
            ...
            'daily calories calculator': function() {
              if (config.mobileDevice == true) {
                  ...
                  return consumerToolsPage
                      .dailyCaloriesCalculator(config)
                      .then(function(bool) {
                          assert.isTrue(bool,
            ...
            'navigable search engine': function() {
                if (config.mobileDevice == true) {
                    this.skip('desktop-only test');
                } else {
                  return consumerToolsPage
            ...
            'navigable nutrition calculators': function() {
                if (config.mobileDevice == true) {
                    this.skip('desktop-only test');
                } else {
                    return consumerToolsPage
                        .nutritionCalculators(config)
                        .then(function(bool) {
                            assert.isTrue(bool,
